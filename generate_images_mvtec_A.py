...
def generate_prompt(added_tokens):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", type=str, default="output_A/fine-tuned/mvtec_ad_setA-0-4_images")
    ...
    # 학습된 토큰만 포함된 프롬프트 생성
    prompts, class_tokens, anomaly_tokens = generate_prompt(added_tokens)
    # 로깅: 토큰 분리 결과 확인
    ...
    for prompt in prompts:
        prompt_dir = os.path.join(args.out_dir, prompt.replace(" ", "_").replace("<", "").replace(">", ""), str(args.guidance_scale))
