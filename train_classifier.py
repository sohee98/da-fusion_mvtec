...
DEFAULT_SYNTHETIC_DIR = "output/textual-inversion-0.5/{dataset}-{seed}-{examples_per_class}"
...
def run_experiment(examples_per_class: int = 0, 
    ...
    # generative_aug : ComposeParallel(TextualInversion(embed_path, ... ))

    if num_synthetic > 0 and aug is not None:
        train_dataset.generate_augmentations(num_synthetic)         # augmentation 이미지 생성 
