...
def generate_class_info(dataset_name):
    ...
    elif dataset_name == 'mvtec_ad_normal':
        class_list = ['metal_nut', 'pill']
        anomaly_dict = {
            'metal_nut': ['scratch', 'color'],
            'pill': ['scratch','color']
...
class MvtecDataset_Normal(FewShotDataset):
    ...
    def __init__(self, args, root_dir=MVTEC_DIR, examples_per_class=None, seed=0, image_size=(256, 256)):
        ...
        for class_name in self.class_list:
            ...
            ### normal 이미지 추출 - train 폴더 
            normal_path = os.path.join(root_dir, class_name, 'train', 'good')
            ...
            self.class_to_images[(class_name, 'good')].extend(image_files)
        ...
        ## class 별 examples_per_class 개수만큼만 저장되어있는 리스트 
        self.all_images = [
    ...
    def get_metadata_by_idx(self, idx: int) -> Dict[str, Any]:
        ...
        if anomaly_type == 'good':
            ...
            is_anomaly = True
            name = f"{class_name}-{anomaly_type}"

        return {"name": name, "path": image_path, 
                "prompt": prompt, "is_anomaly": is_anomaly}
