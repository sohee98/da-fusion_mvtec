...
class FewShotDataset(Dataset):
    ...
    def generate_augmentations(self, num_repeats: int):     # 새로운 이미지 생성 

        self.synthetic_examples.clear()
        options = product(range(len(self)), range(num_repeats))     # coco - 320, 10
        ...
        for idx, num in tqdm(list(
            ...
            image.save(os.path.join(self.synthetic_dir, f"aug-{idx}-origin.png"))       # add - 원본 이미지도 같이 저장 

            image, label = self.generative_aug(
                image, label, self.get_metadata_by_idx(idx))    # image, label, annotation(name,mask,segmentation,area,iscrowd,image_id,bbox,category_id,id 9가지)
